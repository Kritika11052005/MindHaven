...
export default function Home() {
  ...
  // Or useSession(), etc. This should return a boolean
  const { isAuthenticated, user, loading } = useSession();  // Use the session hook
  const router = useRouter();
   const handleJourneyClick = () => {
    console.log('Custom Session - isAuthenticated:', isAuthenticated);
    console.log('Custom Session - user:', user);
    console.log('Custom Session - loading:', loading);
  ...
  const currentEmotions = emotions.find((em) => Math.abs(emotion - em.value) < 15) || emotions[2];
  const features = [
    {
      icon: HeartPulse,
      title: "24/7 Support",
      ...
      <section className="relative min-h-[90vh] mt-20 flex flex-col items-center justify-center py-12 px-4">
        <div className="absolute inset-0 -z-10 overflow-hidden">
          <div
            ...
            bg-gradient-to-r ${currentEmotions.color} to-transparent opacity-60`}
          />
        ...
        >
         ...
         <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold font-plus-jakarta tracking-tight animate-glow">
  <span className="inline-block text-primary">
    Find Peace
  </span>
  <br />
  <span className="inline-block mt-2 text-foreground">
    of Mind
  </span>
</h1>
          ...
          >
            ...
            <div className="relative px-2">
              <div
                className={`absolute inset-0 bg-gradient-to-r ${currentEmotions.color} to-transparent blur-2xl -z-10 transition-all duration-500`}
              />
              <Slider
                value={[emotion]}
