...
export default function TherapyPage() {
  ...
  // Unified function to send messages (used by both form submit and suggested questions)
  const sendMessage = async (text: string, sessionOverride?: string) => {
  ...
  if (!text.trim() || isTyping || isChatPaused || !activeSessionId) {
    console.log("Send message blocked:", {
      noMessage: !text.trim(),
      isTyping,
      isChatPaused,
      noSessionId: !activeSessionId,
    });
    return;
  }

  setIsTyping(true); // ✅ Always set before starting API work
  ...
  try {
    ...
    console.log("stressCheck result:", stressCheck);

    if (stressCheck) {
      setStressPrompt(stressCheck);
      // ❌ Don't set isTyping to false here — keep bar until AI is done
    }
    ...
    setIsTyping(false); // ✅ Always hide bar when done
  }
};
  ...
  // Simplified suggested question handler
  const handleSuggestedQuestion = async (question: string) => {
  console.log("🔥 Button clicked for:", question);
  
  try {
    const sessionId = await createChatSession();
    console.log("✅ New session for suggested question:", sessionId);

    // 🔹 Immediately send the clicked question to AI
    await sendMessage(question, sessionId);

    // 🔹 Optional: Load chat UI for the new session
    //loadChatSession(sessionId);
  } catch (err) {
    console.error("Error starting session from suggested question:", err);
  }
};
  ...
  {/* This is the key change - proper height constraint and overflow handling */}
            ...
            <div className="flex-1 flex items-center justify-center p-4">
              <div className="max-w-2xl w-full space-y-8">
                ...
                <div className="grid gap-3 relative">
                  ...
                  />
                  {(() => {
                    console.log("✅ Suggested questions are being rendered");
                  ...
                  {SUGGESTED_QUESTIONS.map((q, index) => (
                    ...
                    >
                      <button
                        ...
                        className="relative z-10 w-full h-auto py-4 px-6 text-left justify-start hover:bg-muted/50 hover:border-primary/50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed border border-input bg-background rounded-md shadow-sm"
                        onClick={() => {
                          console.log("🔥 CLICK registered for:", q.text);
                          handleSuggestedQuestion(q.text);
                        }}
                        disabled={isTyping || isLoading}
                      >
                        <span className="text-sm font-medium block">{q.text}</span>
                      </button>
                    </motion.div>
                  ))}
